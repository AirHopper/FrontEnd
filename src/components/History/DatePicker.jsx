...
const DatePicker = ({ dateRange, setDateRange, setIsPickerOpen, onSave }) => {
	...
	return (
		<Box position="fixed" top={0} left={0} width="100%" height="100%" bg="rgba(0, 0, 0, 0.3)" zIndex="9">
			<Box position="absolute" top={{ base: '50%' }} left={{ base: '50%' }} transform="translate(-50%, -50%)" transition="all 0.3s ease" bg="white" shadow="lg" zIndex="10" borderRadius="md">
				{/* Tombol X untuk menutup filter dengan lebar penuh */}
				<HStack borderRadius="none" justifySelf="end">
					<CloseButton aria-label="Close Picker" onClick={() => setIsPickerOpen(false)} rounded="none" borderTopRightRadius="md" />
				</HStack>
				...
				<HStack justifyContent="flex-end" p={3}>
					<Button px="35px" colorScheme="blue" onClick={handleSave} bg={'#44B3F8'} _hover={{ bg: '#2078B8' }} borderRadius="xl">
						Simpan
					</Button>
				</HStack>
			</Box>
