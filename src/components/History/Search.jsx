...
const Search = ({ setIsSearchOpen, onSearch }) => {
	...
	return (
		<>
			...
			{/* Search Box */}
			<Box position="fixed" top="50%" left="50%" transform="translate(-50%, -50%)" width="35vw" p="6" px={2} bg="white" shadow="lg" borderRadius="xl" zIndex="10">
					...
					{recentSearches.length > 0 ? (
						<>
							<HStack justify="space-between" px={5} pt={4} width="100%">
								...
								<Text fontWeight="bold" fontSize="sm" color="red.600" cursor="pointer" mr={3} onClick={handleClearAllRecent}>
									Hapus
								</Text>
							</HStack>
							...
							<VStack px={5} align="start" width="100%" gap={1}>
								{recentSearches.map(search => (
									<HStack key={search} justifyContent="space-between" width="100%" borderBottom="1px solid #E2E8F0" pb={1}>
										<Text cursor="pointer" onClick={() => handleSelectRecent(search)} color="blue.500">
											{search}
										</Text>
