...
function CheckoutIndex() {
  ...
  const ticket1Anak = Math.ceil(((parseInt(ticketData?.totalPrice || 0) * 80) / 100)*anak);
  const ticket2Dewasa = parseInt(ticketData?.totalPrice || 0)*dewasa;
  const ticket2Anak = Math.ceil(((parseInt(ticketData?.totalPrice || 0) * 80) / 100)*anak);
  ...
  const [passengerData, setPassengerData] = useState([...dataPenumpangDewasa, ...dataPenumpangAnak]);
      ...
      ...(selectedBerangkat.length > 0 && selectedBerangkat[index]?.id
        ? {
            seatId: [
              ...
              ...(selectedTransit1Berangkat?.[index]?.id ? [selectedTransit1Berangkat[index]?.id] : []),
              ...(selectedTransit2Berangkat?.[index]?.id ? [selectedTransit2Berangkat[index]?.id] : []),
              ...(selectedPulang?.[index]?.id ? [selectedPulang[index]?.id] : []),
              ...(selectedTransit1Pulang?.[index?.id] ? [selectedTransit1Pulang[index]?.id] : []),
              ...(selectedTransit2Pulang?.[index?.id] ? [selectedTransit2Pulang[index]?.id] : []),
            ]
          }
      : {})
    }));
            ...
            >
              <Box>
                  ...
                  <Card.Body marginBottom={5}>
                    <Box>
                      {[...Array(passengerCount)].map((_, index) => (
                        <Box key={index} mb={6}>
                          <Box bg="#3C3C3C" color="white" borderTopRadius="lg" h={10} mb={2}>
                            <Text ml={4} pt={2}>Data Diri Penumpang {index + 1} - {passengerData[index].type}</Text>
                          </Box>
                          <Stack gap="4" px={4}>
                            <SelectRoot
                              collection={listTitle}
                              value={passengerData[index].title}
                              onValueChange={(e) => handleInputChange(index, 'title', e.value)}
                            >
                            ...
                            >
                              <Input
                                ...
                                onChange={(e) => handleInputChange(index, 'namaLengkap', e.target.value)}
                                borderColor="gray.300"
                              />
                            </Field>
                            <Stack 
                            ...
                            {passengerData[index].isSwitchOn && (
                              ...
                              >
                                <Input
                                  ...
                                  onChange={(e) => handleInputChange(index, 'namaKeluarga', e.target.value)}
                                  borderColor="gray.300"
                                />
                              </Field>
                            )}
                            ...
                            >
                              <CalendarComponent
                                ...
                                onChange={(value) => handleInputChange(index, 'tanggalLahir', new Date(value).toISOString())} 
                              />
                            </Field>
                            <Field
                              color="#4B1979"
                            ...
                            >
                              <Input
                                ...
                                onChange={(e) => handleInputChange(index, 'kewarganegaraan', e.target.value)}
                                borderColor="gray.300"
                              />
                            </Field>
                            <Field
                            ...
                            >
                              <Input
                                ...
                                onChange={(e) => handleInputChange(index, 'ktpPas', e.target.value)}
                                borderColor="gray.300"
                              />
                            </Field>
                            <SelectRoot
                              ...
                              onValueChange={(e) => handleInputChange(index, 'negaraPenerbit', e.value)}
                            >
                            ...
                            >
                              <CalendarComponent
                                ...
                                onChange={(date) => handleInputChange(index, 'berlakuSampai', new Date(date).toISOString())}
                              />
                            </Field>
                          </Stack>
                        </Box>
                ...
                >
                  ...
                  {kelas === "Economy" && (
                    <SeatPickerEkonomi setSelected={setSelectedBerangkat} data={someData} seat={seatBerangkat} bookedSeats={bookedSeatBerangkat}/>
                  )}
                  {kelas === "Premium+Economy" && (
                    <SeatPickerPremiunEkonomi setSelected={setSelectedBerangkat} data={someData} seat={seatBerangkat} bookedSeats={bookedSeatBerangkat}/>
                  )}
                  {kelas === "Business" &&(
                    <SeatPickerBisnis setSelected={setSelectedBerangkat} data={someData} seat={seatBerangkat} bookedSeats={bookedSeatBerangkat}/>
                  )}
                  {kelas == "First+Class" && (
                    <SeatPickerEksekutif setSelected={setSelectedBerangkat} data={someData} seat={seatBerangkat} bookedSeats={bookedSeatBerangkat}/>
                  )}
                  {ticketData?.isTransits === true && (
                    ...
                    {kelas === "Economy" && (
                      <SeatPickerEkonomi setSelected={setSelectedTransit1Berangkat} data={someData} seat={seatTransit1Berangkat} bookedSeats={bookedSeatTransit1Berangkat}/>
                    )}
                    {kelas === "Premium+Economy" && (
                      <SeatPickerPremiunEkonomi setSelected={setSelectedTransit1Berangkat} data={someData} seat={seatTransit1Berangkat} bookedSeats={bookedSeatTransit1Berangkat}/>
                    )}
                    {kelas === "Business" &&(
                      <SeatPickerBisnis setSelected={setSelectedTransit1Berangkat} data={someData} seat={seatTransit1Berangkat} bookedSeats={bookedSeatTransit1Berangkat}/>
                    )}
                    {kelas == "First+Class" && (
                      <SeatPickerEksekutif setSelected={setSelectedTransit1Berangkat} data={someData} seat={seatTransit1Berangkat} bookedSeats={bookedSeatTransit1Berangkat}/>
                    )}
                    </>
                  )}
                  {seatTransit2Berangkat.length > 0 && (
                    ...
                    {kelas === "Economy" && (
                      <SeatPickerEkonomi setSelected={setSelectedTransit2Berangkat} data={someData} seat={seatTransit2Berangkat} bookedSeats={bookedSeatTransit2Berangkat}/>
                    )}
                    {kelas === "Premium+Economy" && (
                      <SeatPickerPremiunEkonomi setSelected={setSelectedTransit2Berangkat} data={someData} seat={seatTransit2Berangkat} bookedSeats={bookedSeatTransit2Berangkat}/>
                    )}
                    {kelas === "Business" &&(
                      <SeatPickerBisnis setSelected={setSelectedTransit2Berangkat} data={someData} seat={seatTransit2Berangkat} bookedSeats={bookedSeatTransit2Berangkat}/>
                    )}
                    {kelas == "First+Class" && (
                      <SeatPickerEksekutif setSelected={setSelectedTransit2Berangkat} data={someData} seat={seatTransit2Berangkat} bookedSeats={bookedSeatTransit2Berangkat}/>
                    )}
                    </>
                  )}
                  {ticketData2 && (
                    ...
                    {kelas === "Economy" && (
                      <SeatPickerEkonomi setSelected={setSelectedPulang} data={someData} seat={seatPulang} bookedSeats={bookedSeatPulang}/>
                    )}
                    {kelas === "Premium+Economy" && (
                      <SeatPickerPremiunEkonomi setSelected={setSelectedPulang} data={someData} seat={seatPulang} bookedSeats={bookedSeatPulang}/>
                    )}
                    {kelas === "Business" &&(
                      <SeatPickerBisnis setSelected={setSelectedPulang} data={someData} seat={seatPulang} bookedSeats={bookedSeatPulang}/>
                    )}
                    {kelas == "First+Class" && (
                      <SeatPickerEksekutif setSelected={setSelectedPulang} data={someData} seat={seatPulang} bookedSeats={bookedSeatPulang}/>
                    )}
                    </>
                  )}
                  {ticketData2?.isTransits === true && (
                    ...
                    {kelas === "Economy" && (
                      <SeatPickerEkonomi setSelected={setSelectedTransit1Pulang} data={someData} seat={seatTransit1Pulang} bookedSeats={bookedSeatTransit1Pulang}/>
                    )}
                    {kelas === "Premium+Economy" && (
                      <SeatPickerPremiunEkonomi setSelected={setSelectedTransit1Pulang} data={someData} seat={seatTransit1Pulang} bookedSeats={bookedSeatTransit1Pulang}/>
                    )}
                    {kelas === "Business" &&(
                      <SeatPickerBisnis setSelected={setSelectedTransit1Pulang} data={someData} seat={seatTransit1Pulang} bookedSeats={bookedSeatTransit1Pulang}/>
                    )}
                    {kelas == "First+Class" && (
                      <SeatPickerEksekutif setSelected={setSelectedTransit1Pulang} data={someData} seat={seatTransit1Pulang} bookedSeats={bookedSeatTransit1Pulang}/>
                    )}
                    </>
                  )}
                  {seatTransit2Pulang.length > 0 && (
                    ...
                    {kelas === "Economy" && (
                      <SeatPickerEkonomi setSelected={setSelectedTransit2Pulang} data={someData} seat={seatTransit2Pulang} bookedSeats={bookedSeatTransit2Pulang}/>
                    )}
                    {kelas === "Premium+Economy" && (
                      <SeatPickerPremiunEkonomi setSelected={setSelectedTransit2Pulang} data={someData} seat={seatTransit2Pulang} bookedSeats={bookedSeatTransit2Pulang}/>
                    )}
                    {kelas === "Business" &&(
                      <SeatPickerBisnis setSelected={setSelectedTransit2Pulang} data={someData} seat={seatTransit2Pulang} bookedSeats={bookedSeatTransit2Pulang}/>
                    )}
                    {kelas == "First+Class" && (
                      <SeatPickerEksekutif setSelected={setSelectedTransit2Pulang} data={someData} seat={seatTransit2Pulang} bookedSeats={bookedSeatTransit2Pulang}/>
                    )}
                    </>
                  )}
                  <Box height={5} />
                ...
                >
                  ...
                  <Card.Body>
                    ...
                    <Flex direction="column" marginLeft={7} marginTop={3}>
                      <Text fontWeight="bold">{firstFlight?.airplane} - {ticketData?.class}</Text>
                    </Flex>
                    ...
                    {ticketData2 && (
                      ...
                      />
                      <Flex direction="column" marginLeft={7} marginTop={3}>
                        <Text fontWeight="bold">{firtsReturnFlight?.airplane} - {ticketData?.class}</Text>
                      </Flex>
                      <Flex align="center" marginTop={3}>
                        <FontAwesomeIcon
                          icon={faSun}
