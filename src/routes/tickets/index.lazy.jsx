...
function RouteComponent() {
	...
	const params = {
		...
		departureCity: urlParams.get('search[departureCity]'),
		arrivalCity: urlParams.get('search[arrivalCity]'),
		classType: urlParams.get('search[classType]'),
		continent: urlParams.get('search[continent]'),
		flightDate: urlParams.get('search[flightDate]'),
		orderBy: urlParams.get('orderBy') || 'price_asc',
		isTransit: urlParams.get('search[isTransit]'),
	};
	...
	const updateTickets = selectedDate => {
		...
		getTickets({ ...params, flightDate: selectedDate }).then(data => {
			setTickets(data);
		});
	};
	...
	const handleCheckboxChange = (isDirectChecked, isTransitChecked) => {
		...
		getTickets(updatedParams).then(data => {
			setTickets(data);
		});
	};
								...
								<Box display="block" width="100%">
									...
									<AccordionItemContent>
										<Box display="flex" flexDirection="column" gap={4} py={4}>
											<Box display="flex" flexDirection="column" gap={4} py={4}>
												{/* Checkbox Langsung */}
												<Checkbox isChecked={!isTransitFilter} onChange={e => handleCheckboxChange(e.target.checked, isTransitFilter)} colorScheme="blue">
													Langsung
												</Checkbox>

												{/* Checkbox Transit */}
												<Checkbox isChecked={isTransitFilter} onChange={e => handleCheckboxChange(!e.target.checked, e.target.checked)} colorScheme="blue">
													Transit
												</Checkbox>
											</Box>
										</Box>
